...
public class LimelightValues {
    ...
    public boolean isPoseTrustworthy(Pose2d robotPose) {
        ...
        if ((poseEstimate.getX() < fieldCorner.getX() && poseEstimate.getY() < fieldCorner.getY()) // Don't trust
                                                                                                   // estimations that
                                                                                                   // are outside the
                                                                                                   // field perimeter.
                && robotPose.getTranslation().getDistance(poseEstimate.getTranslation()) < 0.5) // Dont trust pose
